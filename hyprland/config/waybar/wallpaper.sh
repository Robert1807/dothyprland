...
case $1 in
    ...
    "init")
        if [ -f $cache_file ]; then
            ...
            wal -q -i ~/SSD/Pictures/Wallpapers/
        fi
    ;;
    ...
    "select")
        ...
        selected=$( find "$HOME/SSD/Pictures/Wallpapers/" -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" \) -exec basename {} \; | sort -R | while read rfile
        do
            generate_thumbnail "$HOME/SSD/Pictures/Wallpapers/$rfile"  # Generar miniatura
        ...
        wal -q -i ~/SSD/Pictures/Wallpapers/$selected
    ;;
...
# ----------------------------------------------------- 
newwall=$(echo $wallpaper | sed "s|~/SSD/Pictures/Wallpapers/||g")
